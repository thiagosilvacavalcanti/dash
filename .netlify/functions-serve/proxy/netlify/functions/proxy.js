...
async function handler(event) {
  try {
    ...
    const response = await fetch(url, {
      ...
      body: ["GET", "HEAD"].includes(event.httpMethod) ? void 0 : event.body
    });
    ...
    console.error("Proxy error:", e2);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ error: "Proxy error", details: String(e2?.message || e2) })
    };
  }
}
